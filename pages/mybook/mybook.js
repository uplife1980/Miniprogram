      ...
      success: function (res) {
        ...
        for (var i = data.bought.length + data.renting.length + data.outdate.length, j = 0; j < data.rentable.length; i++ , j++) {
          data.rentable[j].title = data.booktitle[i]
            data.rentable[j].showCompleteSheet=0
        }
...
{
  var that=this
  var neverborrow=that.data.neverborrow_list
  neverborrow[e.currentTarget.dataset.index].showCompleteSheet=1
  that.setData({
    neverborrow_list:neverborrow
  })
}
  // // 模态弹窗
  // powerDrawer: function (e) {
